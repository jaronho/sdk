...
""" 系统内核加载网桥模块 """
def loadNetworkBridge():
    os.popen("modprobe br_netfilter")
    lines = os.popen("lsmod | grep br_netfilter").readlines()
    if 0 == len(lines):
        return False
    return True
...
def addIptablesPolicy(ip, port, localIp, localPort, pasvPort):
    # step1: 加载两个内核模块
    os.popen("modprobe ip_nat_ftp")
    os.popen("modprobe ip_conntrack_ftp")
    # step2: 清除所有iptables策略
    os.popen("iptables -F")
    # step3: 添加iptables策略
...
def main():
    if 1 == len(sys.argv):
        sys.argv.append("-h")
    parser = argparse.ArgumentParser(description="用于配置iptables策略",
                                     usage=os.path.basename(__file__)
                                     + " [-h] [-n1 NETPORT1] [-n2 NETPORT2] [-a ADDRESS] [-m NETMASK] [-g GATEWAY]"
                                     + " [-i IP] [-p PORT] [-li LOCAL_IP] [-lp LOCAL_PORT] [-pp PASV_PORT]"
