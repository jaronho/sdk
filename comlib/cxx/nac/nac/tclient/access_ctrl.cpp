...
{
    auto func = [&, callback](int32_t bizCode, int64_t seqId, const Result& ret, const std::string& data) {
        if (!m_bizExecutor)
        {
            if (callback)
            {
                callback(bizCode, seqId, ret, data);
            }
            return;
        }
        auto name = "nac.tcli.api.resp|" + std::to_string(bizCode) + "|" + std::to_string(seqId) + "|" + std::to_string((int)ret);
        m_bizExecutor->post(name, [&, name, bizCode, seqId, ret, data, callback]() {
            if (callback)
            {
                if (m_bizExecutorHook)
                {
                    m_bizExecutorHook(name, [bizCode, seqId, ret, data, callback]() { callback(bizCode, seqId, ret, data); });
                }
                else
                {
                    callback(bizCode, seqId, ret, data);
    ...
    if (!m_sessionManager)
    {
        func(bizCode, seqId, Result::failure, "");
        return -1;
    }
