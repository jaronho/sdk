...
bool AccessCtrl::start(const AccessConfig& cfg)
{
    ...
    if (cfg.retryInterval.size() > 0)
    {
        ...
        {
            m_retryTimer = std::make_shared<threading::SteadyTimer>(std::chrono::seconds(cfg.retryInterval[0]),
                                                                    std::chrono::steady_clock::duration::zero(), "nac.connect.retry",
                                                                    [&]() { onRetryTimer(); });
        }
    }
    /* 首次连接 */
    return m_connectService->connect(cfg.address, cfg.port, cfg.connectTimeout, (unsigned int)cfg.authBizCode, cfg.authTimeout,
                                     (unsigned int)cfg.heartbeatBizCode, cfg.heatbeatInterval);
}
