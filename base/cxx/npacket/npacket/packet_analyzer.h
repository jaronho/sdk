...
public:
    /**
     * @brief 设置层数据回调
     * @param physicalLayerCb 物理层数据回调
     * @param networkLayerCb 网络层数据回调
     * @param transportLayerCb 传输层数据回调
     */
    void setLayerCallback(const LAYER_CALLBACK& physicalLayerCb, const LAYER_CALLBACK& networkLayerCb,
     ...
     * @param data 数据
     * @param dataLen 数据长度
     * @return -1-数据为空, 0-成功, 1-解析物理层失败, 2-解析网络层失败, 3-解析传输层失败
     */
...
private:
     ...
     * @return 协议头部
     */
    std::shared_ptr<ProtocolHeader> handlePhysicalLayer(const uint8_t* data, uint32_t dataLen, uint32_t& headerLen,
...
private:
    LAYER_CALLBACK m_physicalLayerCb = nullptr; /* 物理层数据回调 */
