...
inline IpReassemblyConfig limitIpReassemblyConfig(IpReassemblyConfig cfg)
{
    ...
    /* 限制缓存数量不超过10000条 */
    if (0 == cfg.maxCacheCount || cfg.maxCacheCount > 10000)
    {
        cfg.maxCacheCount = 5000;
    }
    /* 限制单个报文不超过16MB(已远超常规应用层协议需求) */
    if (cfg.maxReassembleSize < 1280 || cfg.maxReassembleSize > (16 * 1024 * 1024)) /* 1280-IPv6最小MTU */
    {
        cfg.maxReassembleSize = (128 * 1024);
    }
    /* 限制单个分片不超过16KB, 超过此值攻击意图明显 */
    if (cfg.maxFragSize < 8 || cfg.maxFragSize > (16 * 1024) || cfg.maxFragSize > cfg.maxReassembleSize - Ipv6Header::getMinLen())
    {
        cfg.maxFragSize = (8 * 1024);
    }
    /* 限制分片数不超过256(RFC 791建议值), 超过极不常见 */
    if (0 == cfg.maxFragCount || cfg.maxFragCount > 256)
    {
        cfg.maxFragCount = 64;
    }
    return cfg;
...
inline TcpReassemblyConfig limitTcpReassemblyConfig(TcpReassemblyConfig cfg)
{
    ...
    if (cfg.maxSegSize < (64 * 1024) || cfg.maxSegSize > (64 * 1024 * 1024))
    {
        cfg.maxSegSize = (4 * 1024 * 1024);
    }
    if (0 == cfg.maxSegCount || cfg.maxSegCount > 10000)
    {
        cfg.maxSegCount = 256;
    }
    if (cfg.maxPendingSize < 1024 || cfg.maxPendingSize > cfg.maxSegSize / 4)
    {
        cfg.maxPendingSize = (128 * 1024);
    }
    return cfg;
}
                             ...
                             int depth, int reassemblyFlag)
{
    ...
    ethernetHeader = handleEthernetLayer(flag, num, data + offset, remainLen, ethHeader, headerLen, networkProtocol);
    if (!ethernetHeader)
    {
        return 1;
    }
    ...
    auto networkHeader = handleNetworkLayer(flag, num, networkProtocol, data + offset, remainLen, ipv4Header, ipv6Header, arpHeader,
    ...
    if (!checkAndHandleFragment(ntp, networkHeader, data + offset, remainLen, outData, isReassembly))
    {
        return 2;
    }
    if (isReassembly)
    {
        if (!outData.empty()) /* 分片已重组完成, 使用重组后的数据继续解析 */
        {
            auto reassembledSource = (NetworkProtocol::IPv4 == networkProtocol ? DataSource::NETWORK_IPv4 : DataSource::NETWORK_IPv6);
            return parseWithDepth(flag, num, ntp, ethernetHeader, outData.data(), outData.size(), reassembledSource, depth + 1, 1);
        }
        return 5; /* 分片未收齐, 等待后续 */
    }
                                    ...
                                    const DataSource& dataSource, int depth, int reassemblyFlag)
{
    ...
    {
        networkProtocol = NetworkProtocol::IPv4;
    }
    else if (DataSource::NETWORK_IPv6 == dataSource)
    {
        networkProtocol = NetworkProtocol::IPv6;
    }
    else
    {
        return 2;
    ...
    std::vector<uint8_t> outData;
    bool isReassembly = false;
    if (!checkAndHandleFragment(ntp, networkHeader, data, dataLen, outData, isReassembly))
    {
        return 2;
    }
    if (isReassembly)
    {
        if (!outData.empty()) /* 分片已重组完成, 使用重组后的数据继续解析 */
        {
            return parseWithDepth(flag, num, ntp, ethernetHeader, outData.data(), outData.size(), dataSource, depth + 1, 1);
        }
        return 5;
    }
                                            ...
                                            uint32_t dataLen, int depth, int reassemblyFlag)
{
    ...
    uint32_t headerLen = 0;
    auto transportHeader =
        handleTransportLayer(flag, num, transportProtocol, data, dataLen, tcpHeader, udpHeader, icmpHeader, icmpv6Header, headerLen);
    if (!transportHeader)
    {
        return 3;
    }
    ...
    {
        const Ipv4Header* ipv4Header = (const Ipv4Header*)(networkHeader);
        if (ipv4Header->totalLen >= ipv4Header->headerLen)
        {
            uint32_t ipPayloadLen = ipv4Header->totalLen - ipv4Header->headerLen;
            if (ipPayloadLen >= headerLen && ipPayloadLen <= dataLen)
            {
                payloadLen = ipPayloadLen - headerLen;
            }
        }
        if (TransportProtocol::UDP == transportProtocol) /* 对于UDP, 优先使用UDP.length(如果有效), 因为它更精确 */
        {
            const UdpHeader* udpHeader = (const UdpHeader*)(transportHeader);
            if (udpHeader->totalLen >= UdpHeader::getMinLen())
            {
                uint32_t udpPayloadLen = udpHeader->totalLen - UdpHeader::getMinLen();
                payloadLen = (udpPayloadLen <= remainLen) ? udpPayloadLen : remainLen;
            }
        }
    }
    else if (NetworkProtocol::IPv6 == networkHeader->getProtocol())
    {
        ...
        if (ipv6->payloadLen >= headerLen)
        {
            payloadLen = ipv6->payloadLen - headerLen; /* headerLen应包含TCP头+Extension Headers */
        }
        else /* 长度字段异常 */
        {
            payloadLen = 0;
    ...
    if (TransportProtocol::TCP == transportProtocol) /* 如果是TCP, 进行TCP流重组 */
    {
        TcpStreamKey tcpKey;
        bool needMoreData = false;
        std::vector<uint8_t> outData;
        bool isReassembly = false;
        if (!checkAndHandleTcpStream(ntp, networkHeader, transportHeader, payload, payloadLen, tcpKey, needMoreData, outData, isReassembly))
        {
            return 3;
        }
        if (!outData.empty()) /* 有重组后的数据, 解析应用层, 并传入tcpKey用于关联状态 */
        {
            if (isReassembly && reassemblyFlag < 2) /* 重组数据 */
            {
                reassemblyFlag += 2;
            }
            return handleApplicationLayer(flag, num, ntp, totalLen, transportHeader, outData.data(), outData.size(), &tcpKey, depth,
                                          reassemblyFlag);
        }
...
ProtocolHeader* Analyzer::handleNetworkLayer(size_t flag, size_t num, uint32_t networkProtocol, const uint8_t* data, uint32_t dataLen,
...
ProtocolHeader* Analyzer::handleTransportLayer(size_t flag, size_t num, uint32_t transportProtocol, const uint8_t* data, uint32_t dataLen,
    ...
    for (const auto& parser : parserList) /* 初始化每个解析器的执行状态 */
    {
        ...
        if (streamInfo) /* 检查是否有该解析器的历史状态(TCP流场景) */
        {
                ...
                && !iterPending->second.empty())
            {
                ...
                {
                    context.pendingData.insert(context.pendingData.end(), payload, payload + payloadLen);
                }
...
void Analyzer::cleanupFragmentCache(const std::chrono::steady_clock::time_point& ntp)
{
    ...
    size_t clearInterval = m_ipReassemblyCfg.timeout / 10; /* 使用(超时时间/10)作为清理间隔 */
    if (clearInterval < 1000)
    {
        clearInterval = 1000;
    }
    if (std::chrono::duration_cast<std::chrono::milliseconds>(ntp - m_lastCleanupTime).count() <= clearInterval)
    {
        return;
    }
    ...
    for (auto iter = m_fragmentCache.begin(); m_fragmentCache.end() != iter;)
    {
        ...
        if (elapsed > m_ipReassemblyCfg.timeout) /* 清理超时分片缓存 */
        {
...
bool Analyzer::checkAndHandleFragment(const std::chrono::steady_clock::time_point& ntp, const ProtocolHeader* networkHeader,
                                      const uint8_t* data, uint32_t dataLen, std::vector<uint8_t>& outData, bool& isReassembly)
{
    outData.clear();
    isReassembly = false;
    ...
    if (m_fragmentCache.end() == iter) /* 新分片 */
    {
        auto fragmentInfo = std::make_unique<FragmentInfo>();
        info = fragmentInfo.get();
        info->originalProtocol = originalProtocol; /* 保存原始协议 */
        iter = m_fragmentCache.insert(std::make_pair(key, std::move(fragmentInfo))).first;
    }
    else /* 旧分片 */
    {
    ...
    if (info->fragmentCount >= m_ipReassemblyCfg.maxFragCount) /* 检查分片数量(疑似DoS攻击) */
    {
    ...
    /* 重组数据 */
    outData.reserve(basicHeaderLen + info->totalLen);
    outData.insert(outData.end(), data, data + basicHeaderLen); /* 先复制IP头部 */
    /* 检查分片连续性 */
    ...
    for (auto& kv : info->fragments)
    {
        ...
        outData.insert(outData.end(), kv.second.begin(), kv.second.end());
    ...
    if (isIpv4) /* IPv4 */
    {
        uint16_t newTotalLen = (uint16_t)(outData.size());
        auto ipHeader = (RawIpv4Header*)(outData.data());
    ...
    else /* IPv6 */
    {
        ...
        uint16_t newPayloadLen = (uint16_t)(outData.size() - Ipv6Header::getMinLen());
        auto ipHeader = (RawIpv6Header*)(outData.data());
...
void Analyzer::cleanupTcpStreamCache(const std::chrono::steady_clock::time_point& ntp)
{
    ...
    size_t clearInterval = m_tcpReassemblyCfg.timeout / 10; /* 使用(超时时间/10)作为清理间隔 */
    if (clearInterval < 1000)
    {
        clearInterval = 1000;
    }
    if (std::chrono::duration_cast<std::chrono::milliseconds>(ntp - m_lastTcpCleanupTime).count() <= clearInterval)
    {
        return;
    }
    ...
    for (auto iter = m_tcpStreamCache.begin(); iter != m_tcpStreamCache.end();)
    {
        ...
        if (elapsed > m_tcpReassemblyCfg.timeout) /* 清理超时的流 */
        {
...
bool Analyzer::checkAndHandleTcpStream(const std::chrono::steady_clock::time_point& ntp, const ProtocolHeader* networkHeader,
                                       ...
                                       TcpStreamKey& key, bool& needMoreData, std::vector<uint8_t>& outData, bool& isReassembly)
{
    needMoreData = false;
    outData.clear();
    isReassembly = false;
    ...
    {
        const Ipv4Header* ipv4Header = (const Ipv4Header*)(networkHeader);
        key = TcpStreamKey::createIpv4(ipv4Header->srcAddr, ipv4Header->dstAddr, tcpHeader->srcPort, tcpHeader->dstPort);
    }
    else if (NetworkProtocol::IPv6 == networkHeader->getProtocol())
    {
    ...
    {
        if (tcpHeader->flagRst || tcpHeader->flagFin) /* RST或FIN-ACK重传到达时, 流可能已被清理, 直接返回当前数据(如果有) */
        {
            if (payloadLen > 0)
            {
        ...
        iter = m_tcpStreamCache.insert(std::make_pair(key, std::move(streamInfo))).first;
    }
    info = iter->second.get();
    if (!info) /* 防御性检查: 防止map中存在空指针 */
    {
    ...
    {
        /* 追加当前FIN包数据 */
        if (payloadLen > 0 && (outData.empty() || tcpHeader->seq == info->nextExpectedSeq))
        {
            outData.insert(outData.end(), payload, payload + payloadLen);
            if (info->isSeqInitialized)
            {
                info->nextExpectedSeq = seqAdd(info->nextExpectedSeq, payloadLen);
            }
        }
        /* FIN时无条件合并所有缓存的乱序数据 */
        if (!info->segments.empty())
        {
            ...
            for (const auto& p : sortedSegs)
            {
                outData.insert(outData.end(), p.second.data.begin(), p.second.data.end());
            }
            info->segmentsContinueCount = 0;
            info->segments.clear();
            isReassembly = true;
        }
    ...
    if (!info->isSeqInitialized) /* 初始化序列号 */
    {
        ...
        info->nextExpectedSeq = seqAdd(initialSeq, payloadLen);
        if (payloadLen > 0) /* 即使没有payload, 也标记为已初始化, 但不等待更多数据 */
        {
            outData.insert(outData.end(), payload, payload + payloadLen);
            return true;
        }
        if (tcpHeader->flagSyn && !tcpHeader->flagFin && !tcpHeader->flagRst && 0 == payloadLen) /* 纯SYN包, 不需要等待数据 */
        {
            needMoreData = false;
        }
        else /* 只有当这是真正的数据流开始时才需要等待 */
        {
            needMoreData = true;
        }
        return true;
    }
    uint32_t curSeq = tcpHeader->seq;
    if (seqLt(curSeq, info->nextExpectedSeq)) /* 序列号重传/重叠处理 */
    {
        /* 重复包或旧数据 */
        uint32_t endSeq = seqAdd(curSeq, payloadLen);
        if (seqLe(endSeq, info->nextExpectedSeq)) /* 完全重复 */
        {
            needMoreData = false;
            outData.clear(); /* 无数据输出给业务 */
            return false; /* 丢弃重复数据 */
        }
        /* 部分重叠, 截取新数据 */
        uint32_t offset = info->nextExpectedSeq - curSeq;
        if (offset < payloadLen)
        {
            payload += offset;
            payloadLen -= offset;
            curSeq = info->nextExpectedSeq;
        }
        else
        {
            if (payloadLen > 0)
    ...
    if (curSeq == info->nextExpectedSeq) /* 按序到达 */
    {
        info->nextExpectedSeq = seqAdd(curSeq, payloadLen);
        info->segmentsContinueCount = 0;
        if (payloadLen > 0)
        {
            outData.insert(outData.end(), payload, payload + payloadLen);
        ...
        /* 整合乱序分段 */
        bool hasMore = true;
        while (hasMore && !info->segments.empty())
        {
            hasMore = false;
            /* 收集所有可以合并的key(curSeq <= nextExpectedSeq) */
            std::vector<uint32_t> toMerge;
            for (const auto& kv : info->segments)
            {
                if (seqLe(kv.first, info->nextExpectedSeq))
                {
                    toMerge.push_back(kv.first);
                }
            }
            if (toMerge.empty())
            {
            ...
            {
                auto it = info->segments.find(cachedSeq);
                if (info->segments.end() == it)
                {
                    continue;
                }
                ...
                if (seqLe(cachedSeq, info->nextExpectedSeq))
                {
                    uint32_t offset = 0;
                    if (seqLt(cachedSeq, info->nextExpectedSeq))
                    {
                        offset = info->nextExpectedSeq - cachedSeq;
                    }
                    if (offset < segment.data.size())
                    {
                        outData.insert(outData.end(), segment.data.begin() + offset, segment.data.end());
                        info->nextExpectedSeq = seqAdd(info->nextExpectedSeq, (uint32_t)(segment.data.size() - offset));
                    }
                    info->segments.erase(it);
                    hasMore = true;
                }
    ...
    {
        info->segmentsContinueCount += 1;
        /* 统计当前缓存总字节 */
        size_t totalCacheSize = 0;
        for (const auto& seg : info->segments)
        {
            if (totalCacheSize > SIZE_MAX - seg.second.data.size())
            {
                break;
            }
            totalCacheSize += seg.second.data.size();
        }
        /* 缓存超限逻辑: 内存/分段数任一达到上限, 优先输出头部连续分段, 无连续则淘汰最旧分段 */
        bool cacheFull = (totalCacheSize + payloadLen >= m_tcpReassemblyCfg.maxSegSize - m_ipReassemblyCfg.maxFragSize)
                         || (info->segmentsContinueCount >= m_tcpReassemblyCfg.maxSegCount);
        if (cacheFull && !info->segments.empty()) /* 缓存满: 优先输出头部连续分段, 无连续则淘汰最小seq分段 */
        {
            std::vector<std::pair<uint32_t, TcpSegment>> sortedSegs(info->segments.begin(), info->segments.end());
            std::sort(sortedSegs.begin(), sortedSegs.end(), [](const auto& a, const auto& b) { return seqLt(a.first, b.first); });
            uint32_t expectSeq = info->nextExpectedSeq;
            std::vector<phmap::flat_hash_map<uint32_t, TcpSegment>::iterator> continuousList;
            for (const auto& p : sortedSegs)
            {
                if (p.first == expectSeq)
                {
                    auto it = info->segments.find(p.first);
                    if (info->segments.end() != it)
                    {
                        continuousList.push_back(it);
                        expectSeq = seqAdd(p.first, p.second.payloadLen);
                    }
                }
                else if (seqLt(p.first, expectSeq)) /* 该分段的seq小于期望seq, 说明是重复或已覆盖的数据, 跳过 */
                {
                    continue;
                }
                else /* 不连续, 停止收集 */
            ...
            if (continuousList.empty()) /* 无连续分段, 兜底淘汰最小seq(最旧)分段 */
            {
                uint32_t minSeq = sortedSegs[0].first;
                auto minIter = info->segments.find(minSeq);
                if (info->segments.end() != minIter)
                {
                    info->segments.erase(minIter);
                }
            }
            else /* 存在连续分段, 重组上交业务 */
            {
                ...
                {
                    outData.insert(outData.end(), it->second.data.begin(), it->second.data.end());
                    lastEndSeq = seqAdd(it->first, it->second.payloadLen);
                    info->segments.erase(it); /* 删除已输出分段 */
                }
                info->nextExpectedSeq = lastEndSeq;
            }
        }
        if (payloadLen > 0) /* 存储当前乱序分段 */
        {
            TcpSegment segment;
            segment.seq = curSeq;
            ...
            auto segIt = info->segments.find(curSeq);
            if (info->segments.end() == segIt)
            {
                info->segments[curSeq] = std::move(segment);
            }
            else if (segment.payloadLen > segIt->second.payloadLen) /* 已存在分片, 仅当新分片更长时覆盖, 否则丢弃当前残缺重传包 */
            {
                segIt->second = std::move(segment);
            }
            needMoreData = true;
            isReassembly = true;
        }
