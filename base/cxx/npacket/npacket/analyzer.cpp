                                            ...
                                            uint32_t dataLen, int depth)
{
    ...
    if (TransportProtocol::TCP == transportProtocol) /* 如果是TCP, 进行TCP流重组 */
    {
        ...
        auto reassembled = checkAndHandleTcpReassembly(ntp, networkHeader, transportHeader, payload, payloadLen, tcpKey, needMoreData);
        if (reassembled && !reassembled->empty()) /* 有重组后的数据, 解析应用层, 并传入tcpKey用于关联状态 */
        {
            return handleApplicationLayer(num, ntp, totalLen, transportHeader, reassembled->data(), reassembled->size(), &tcpKey, depth);
        }
                                     ...
                                     int depth)
{
    ...
    /* step2. 获取候选解析器 */
    std::vector<std::weak_ptr<ProtocolParser>> parserList;
    {
        std::lock_guard<std::mutex> locker(m_mutexParserList);
        ...
        {
            bool findFlag = false;
            for (const auto& wpParser : parserList)
            {
    ...
    if (streamInfo && !streamInfo->wpWaitingParserList.empty())
    {
        ...
        for (const auto& wpParser : streamInfo->wpWaitingParserList)
        {
            ...
            for (const auto& cp : parserList)
            {
                if (cp.lock() == parser)
                {
                    stillValid = true;
                    tempParserList.emplace_back(wpParser);
                    break;
                }
            }
        }
        ...
        for (const auto& cp : parserList)
        {
            ...
            if (!isWaiting)
            {
                newCandidates.emplace_back(cp);
            }
        }
        parserList = std::move(newCandidates);
    }
    /* step5. 应用层协议解析 */
    ...
    while (offset < fullDataLen)
    {
        ...
        for (const auto& wpParser : parserList) /* 尝试所有候选解析器 */
        {
            ...
            {
                continueParsers.emplace_back(parser);
            }
            /* FAILURE, 继续尝试下一个 */
        }
