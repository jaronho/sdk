...
public:
     ...
     * @param timeout 超时读取(毫秒), 必须>0, 表示超过该时间就返回缓冲区所有包(不管缓冲区有没有满)
     * @param bufferSize 接收缓冲区大小(字节), >=8MB时自动启用TPACKET_V3
     * @return true-成功, false-失败
     */
    bool open(const std::string& name, int direction = 0, int snapLen = 0, int promisc = 1, int timeout = 1000, int bufferSize = 0);
