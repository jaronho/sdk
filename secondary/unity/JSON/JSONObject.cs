...
//Using doubles will cause errors in some situations
	...
	void Parse(string str, int maxDepth = -2, bool storeExcessLevels = false, bool strict = false) {
		if(!string.IsNullOrEmpty(str)) {
			...
			if(strict) {
				if(str[0] != '[' && str[0] != '{') {
					type = Type.NULL;
					Debug("Improper (strict) JSON formatting.  First character must be [ or {");
					return;
				}
			}
			if(str.Length > 0) {
					...
					switch(str[offset]) {
						...
						default:
							try {
								...
								type = Type.NULL;
								Debug("improper JSON formatting:" + str);
							}
							return;
					}
					string propName = "";
	...
	static void MergeRecur(JSONObject left, JSONObject right) {
		if(left.type == Type.NULL)
			...
			if(right.Count > left.Count) {
				Debug("Cannot merge arrays when right object has more elements");
				return;
			}
			for(int i = 0; i < right.list.Count; i++) {
		...
		if(type == Type.OBJECT) {
			...
			for(int i = 0; i < list.Count; i++) {
				...
				switch(val.type) {
					...
					default:
						Debug("Omitting object: " + keys[i] + " in dictionary conversion");
						break;
				}
			}
			return result;
		}
		Debug("Tried to turn non-Object JSONObject into a dictionary");
		return null;
	}
