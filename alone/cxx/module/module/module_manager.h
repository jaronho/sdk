...
#define REGISTER_MODULE(moduleType) \
    static bool s_module_##moduleType##_registered = module::ModuleManager::getInstance().registerCreator( \
        typeid(moduleType), []() -> std::shared_ptr<module::Module> { return std::make_shared<moduleType>(); })

/* 注册模块, 只能在cpp文件中使用 */
#define REGISTER_MODULE_IMPL(moduleType, moduleTypeImpl) \
    static bool s_module_##moduleType##_registered = module::ModuleManager::getInstance().registerCreator( \
        typeid(moduleType), []() -> std::shared_ptr<module::Module> { return std::make_shared<moduleTypeImpl>(); })

/* 获取模块 */
#define GET_MODULE(moduleType) module::ModuleManager::getInstance().get<moduleType>()
