...
public:
    ...
    static short swab16(short n);                                              /* 大小端转换(短整型数字转为短整型数字) */
    static short swab16_array(unsigned char* p);                               /* 大小端转换(字节流转为短整型数字) */

    static int swab32(int n);                                                  /* 大小端转换(整型数字转为整型数字) */
    static int swab32_array(unsigned char* p);                                 /* 大小端转换(字节流转为整型数字) */

    static int def_size(void);                                                 /* 默认字节流长度 */
...
public:
    ...
    const unsigned char* getContent(void);                                     /* 获取字节流内容 */
    bool setContent(const unsigned char* content, int len);                    /* 设置字节流内容 */
...
public:
    ...
    char* read_string(unsigned int* len);                                      /* 从字节流读取字符串 */
    bool write_string(const char* str, unsigned int len);                      /* 向字节流写入字符串 */
...
private:
    unsigned char* mContent;                                                   /* 字节流内容指针 */
