...
unsigned int BaseConvert::hex2dec(const std::string& hexStr) {
	...
	for (size_t i = start; i < len; ++i) {
		const char& ch = hexStr.at(i);
		if (ch >= '0' && ch <= '9') {
			dec += (ch - 48) * static_cast<unsigned int>(pow(16, len - i - 1));
		} else if (ch >= 'A' && ch <= 'F') {
			dec += (ch - 55) * static_cast<unsigned int>(pow(16, len - i - 1));
		} else if (ch >= 'a' && ch <= 'f') {
			dec += (ch - 87) * static_cast<unsigned int>(pow(16, len - i - 1));
		} else {
			return 0;
		}
	}
