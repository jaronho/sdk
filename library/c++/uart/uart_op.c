...
int uart_set_baud_rate(int fd, unsigned int baudRate) {
    struct termios opt;
    unsigned int speed;
    if (fd < 0) {
        return 1;
...
int uart_set_data_bits(int fd, unsigned int bits) {
    struct termios opt;
    if (fd < 0) {
        return -1;
    }
...
int uart_set_parity(int fd, char parity) {
    ...
    /* PARENB:校验位使能,PARODD:使用奇校验而不使用偶校验,INPCK:奇偶校验使能,ISTRIP:除去奇偶校验位 */
    switch (parity) {
    case 'N':
...
int uart_set_stop_bits(int fd, int bits) {
    struct termios opt;
    if (fd < 0) {
        return -1;
    }
