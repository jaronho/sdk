...
public class Formula {
	...
	public static float calcDistance(float x1, float y1, float x2, float y2) {
		float x = (float)System.Math.Abs(x2 - x1);
		float y = (float)System.Math.Abs(y2 - y1);
	...
	public static float calcVectorAngle(float x1, float y1, float x2, float y2) {
		float x = x2 - x1;
		float y = y2 - y1;
		float side = (float)System.Math.Sqrt(x * x + y * y);
		float radian = (float)System.Math.Acos(x / side);
		float angle = 180 / (float)(System.Math.PI / radian);
		if (y < 0) {
			angle = 360 - angle;
		} else if (0 == y && x < 0) {
			angle = 180;
     ...
     * 返回值: float
     */
	public static float calcTriangleAngle(float aX, float aY, float bX, float bY, float cX, float cY, int angleType = 1) {
		...
		if (1 == angleType) {			// 计算角A
			...
			angle = (float)(System.Math.Acos(ca) * 180 / System.Math.PI);
		} else if (2 == angleType) {	// 计算角B
			float cb = (a * a - b * b + c * c) / (2 * c * a);
			angle = (float)(System.Math.Acos(cb) * 180 / System.Math.PI);
		} else if (3 == angleType) {	// 计算角C
			float cc = (a * a + b * b - c * c) / (2 * a * b);
			angle = (float)(System.Math.Acos(cc) * 180 / System.Math.PI);
		}
		angle = float.IsNaN(angle) ? 0 : angle;
		return angle;
	}
     ...
     */
	 public static float[] calcPointOnCircleByAngle(float cX, float cY, float r, float angle) {
        ...
        if (0 == angle || 360 == angle) {
            ...
            x = cX + r * (float)System.Math.cos(angle * System.Math.PI / 180);
            y = cY + r * (float)System.Math.sin(angle * System.Math.PI / 180);
        } else if (90 == angle) {
            x = cX;
            y = cY + r;
        } else if (angle > 90 && angle < 180) {     // 第2象限
            x = cX - r * (float)System.Math.cos((180 - angle) * System.Math.PI / 180);
            y = cY + r * (float)System.Math.sin((180 - angle) * System.Math.PI / 180);
        } else if (180 == angle) {
            x = cX - r;
            y = cY;
        } else if (angle > 180 && angle < 270) {    // 第3象限
            x = cX - r * (float)System.Math.cos((angle - 180) * System.Math.PI / 180);
            y = cY - r * (float)System.Math.sin((angle - 180) * System.Math.PI / 180);
        } else if (270 == angle) {
            x = cX;
            y = cY - r;
        } else if (angle > 270 && angle < 360) {    // 第4象限
            x = cX + r * (float)System.Math.cos((360 - angle) * System.Math.PI / 180);
            y = cY - r * (float)System.Math.sin((360 - angle) * System.Math.PI / 180);
        }
    ...
    public static float[] calcPointOnRectangleByAngle(float aX, float aY, float w1, float w2, float h1, float h2, float angle) {
        angle = angle % 360;
        angle = angle >= 0 ? angle : 0;
        float tmpAngle1 = (float)(System.Math.atan(h1 / w2) * 180 / System.Math.PI);
        float tmpAngle2 = (float)(System.Math.atan(h1 / w1) * 180 / System.Math.PI);
        float tmpAngle3 = (float)(System.Math.atan(h2 / w1) * 180 / System.Math.PI);
        float tmpAngle4 = (float)(System.Math.atan(h2 / w2) * 180 / System.Math.PI);
        float x = 0;
        float y = 0;
        if (0 == angle || 360 == angle) {
            x = aX + w2;
            ...
            x = aX + w2;
            y = aY + (float)(System.Math.tan(angle * System.Math.PI / 180) * w2);
        } else if (angle == tmpAngle1) {
            x = aX + w2;
            y = aY + h1;
        } else if (angle > tmpAngle1 && angle < 90) {
            x = aX + (float)(System.Math.tan((90 - angle) * System.Math.PI / 180) * h1);
            y = aY + h1;
        } else if (90 == angle) {
            x = aX;
            y = aY + h1;
        } else if (angle > 90 && angle < 180 - tmpAngle2) {
            x = aX - (float)(System.Math.tan((angle - 90) * System.Math.PI / 180) * h1);
            y = aY + h1;
            ...
            x = aX - w1;
            y = aY + (float)(System.Math.tan((180 - angle) * System.Math.PI / 180) * w1);
        } else if (180 == angle) {
            x = aX - w1;
            y = aY;
        } else if (angle > 180 && angle < 180 + tmpAngle3) {
            x = aX - w1;
            y = aY - (float)(System.Math.tan((angle - 180) * System.Math.PI / 180) * w1);
        } else if (180 + tmpAngle3 == angle) {
            x = aX - w1;
            y = aY - h2;
        } else if (angle > 180 + tmpAngle3 && angle < 270) {
            x = aX - (float)(System.Math.tan((270 - angle) * System.Math.PI / 180) * h2);
            y = aY - h2;
        } else if (270 == angle) {
            x = aX;
            y = aY - h2;
        } else if (angle > 270 && angle < 360 - tmpAngle4) {
            x = aX + (float)(System.Math.tan((angle - 270) * System.Math.PI / 180) * h2);
            y = aY - h2;
            ...
            x = aX + w2;
            y = aY - (float)(System.Math.tan((360 - angle) * System.Math.PI / 180) * w2);
        }
        float[] point = new float[2];
        point[0] = x;
        point[1] = y;
     ...
     * 返回值: boolean
     */
	public static bool isPointInIntercept(float aX, float aY, float bX, float bY, float cX, float cY, float x, float y) {
     ...
     * 返回值: boolean
     */
	public static bool isPointInRect(float rectX, float rectY, float rectW, float rectH, float x, float y) {
     ...
     * 返回值: boolean
     */
	public static bool isPointInTriangle(float aX, float aY, float bX, float bY, float cX, float cY, float x, float y) {
		bool temp1 = isPointInIntercept(aX, aY, bX, bY, cX, cY, x, y);
		bool temp2 = isPointInIntercept(aX, aY, cX, cY, bX, bY, x, y);
		bool temp3 = isPointInIntercept(bX, bY, cX, cY, aX, aY, x, y);
     ...
     * 返回值: boolean
     */
	public static bool isPointInCircle(float cX, float cY, float r, float x, float y) {
	...
	public static double[] convertLngLatToMercator(double lng, double lat) {
		...
		double y = System.Math.Log(System.Math.Tan((90 + lat) * System.Math.PI / 360)) / (System.Math.PI / 180);
     ...
     * 返回值: float[]
     */
	public static float[] calcSegmentIntersection(float aX, float aY, float bX, float bY, float cX, float cY, float dX, float dY, float limit = 0.1f, bool mustIn = false) {
		...
		if (System.Math.Abs(denominator) <= System.Math.Abs(limit)) {
			return xy;
		}
		// 线段所在直线的交点坐标
     ...
     * 返回值: float[]
     */
	public static float[] calcCardinalSplinePointAt(float p0X, float p0Y, float p0Z, float p1X, float p1Y, float p1Z, float p2X, float p2Y, float p2Z, float p3X, float p3Y, float p3Z, float t, float s = 0.5f) {
		float t2 = t * t;
		float t3 = t2 * t;
